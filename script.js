// TO DO
// read data from json
// change library used to get font info
// change units to dvh for mobile
// presentation mode
// move controls to left side of the screen
// add course info to the right side
// allow sorting/filtering in font list
// x ray view
// change alphabetizer to show off multiple instances with diff variable amounts instead of same thing over and over
// remove alphabetizer, make a full glyph set instrument
...
// Fetch JSON
let fontMetadata, fontMetadataKeys;
async function fetchJSON() {
	const request = new Request("data.json");
	const response = await fetch(request);
	fontMetadata = await response.json();
	fontMetadataKeys = Object.keys(fontMetadata);
	generateFontMenu();
}
fetchJSON();

function generateFontMenu() {
	let tempFontMenuHTML = "";
	let tempFontCSS = "";
	for (let fontName of fontMetadataKeys) {
		let fontInfo = fontMetadata[fontName];
		tempFontMenuHTML += `
			<li class="menu-fonts-grid-item" data-font="${fontName}" onclick="pickFont('${fontName}')" onmouseenter="playTom('random')">
				<svg class="corner-circle-1" viewBox="0 0 20 20"><circle cx="10" cy="10" r="10"/></svg><svg class="corner-circle-2" viewBox="0 0 20 20"><circle cx="10" cy="10" r="10"/></svg><svg class="corner-circle-3" viewBox="0 0 20 20"><circle cx="10" cy="10" r="10"/></svg><svg class="corner-circle-4" viewBox="0 0 20 20"><circle cx="10" cy="10" r="10"/></svg>
				<span class="menu-fonts-grid-item-preview">${fontInfo["menuPreview"]}</span>
				<span class="menu-fonts-grid-item-info">
					<strong>${fontInfo["menuName"]}</strong><br>
					by ${fontInfo["menuAuthor"]}
				</span>
			</li>
		`
		tempFontCSS += `
			@font-face {
				...
				src: url("${fontInfo["exportFilepath"]}");
			}
			@keyframes preview-${fontName} {
				${fontInfo["menuKeyframes"]}
				0% {
					font-variation-settings: "wght" 400, "ital" 0.00;
				}
				100% {
					font-variation-settings: "wght" 900, "ital" 2.00;
				}
			}
			[data-font="${fontName}"] .menu-fonts-grid-item-preview {
				font-family: ${fontName};
			}
			[data-font="jost"]:hover .menu-fonts-grid-item-preview {
				animation: ${fontInfo["menuDuration"]} preview-${fontName} infinite alternate ease-in-out;
			}
		`
	}
	let fontboxFontsDOM = document.querySelector("#menu-fonts-grid");
	fontboxFontsDOM.innerHTML = tempFontMenuHTML;
	document.head.insertAdjacentHTML("beforeend", `<style>${tempFontCSS}</style>`)
}

let sortingModes = ["Random", "Font A–Z", "Font Z–A", "Designer A–Z", "Designer Z–A"];
let sortingIndex = 0;
let filterModes = ["Show All", "Sans", "Serif", "Pixel", "Decorative", "Illustration", "Remixed", "Original"];
let filterIndex = 0;

function toggleSorting() {
	sortingIndex++;
	if (sortingIndex >= sortingModes.length) {
		sortingIndex = 0;
	}
	let sortingDisplay = document.querySelector("#menu-fonts-sorting-display");
	sortingDisplay.innerText = sortingModes[sortingIndex];
}
function toggleFilter() {
	filterIndex++;
	if (filterIndex >= filterModes.length) {
		filterIndex = 0;
	}
	let filterDisplay = document.querySelector("#menu-fonts-filter-display");
	filterDisplay.innerText = filterModes[filterIndex];
}
...
function fontsIn() {
	...
	let grid = fontMenu.querySelectorAll('li');
